...
* Use Upstash Redis/KV as persistent counter store.
* Enforce rate limit in `/api/booking` Route Handler.
...
In Next.js App Router, this is implemented as a Route Handler:
...
| `slug`              | text        | Slug unik untuk paket                |
| `type`              | enum        | `mobil_kursus` atau `mobil_sendiri`  |
| `price`             | numeric     | Harga paket dalam Rupiah             |
| `sessions_count`    | integer     | Jumlah total sesi dalam paket        |
| `duration_minutes`  | integer     | Durasi per sesi, default 90          |
| `short_description` | text        | Deskripsi singkat untuk card         |
| `description`       | text        | Deskripsi lengkap                    |
| `is_active`         | boolean     | Hanya paket aktif yang tampil publik |
| `is_featured`       | boolean     | Untuk highlight paket tertentu       |
...
| ------------------ | ----------- | ---------------------------------------- |
| `id`               | uuid        | Primary key                              |
| `name`             | text        | Nama customer/siswa                      |
| `phone`            | text        | Nomor WhatsApp display/original cleaned  |
| `normalized_phone` | text        | Format normalized `62xxxxxxxxxx`, unique |
...
* Public app upsert by `normalized_phone`.
* `normalized_phone` should be unique.
...
| Data Fetching    | Server Components / Route Handlers / direct Supabase client |
| State Management | Local state only for MVP                                    |
| TanStack Query   | Not used in MVP                                             |
| Theme            | Light mode only                                             |
| Rate Limit       | Upstash Ratelimit + Redis/KV                                |
| WhatsApp         | WAHA or GOWA REST API, non-blocking                         |
| Testing          | Playwright                                                  |
| Deployment       | Vercel                                                      |
...
* Server Component.
* Fetch active packages server-side.
* Use ISR/revalidation if package data is not frequently updated.
* SEO metadata generated at page level.
...
* Client Component for form wizard UI.
* Server fetch or route call for initial package data.
...
Optional internal Route Handler or server function.

Responsibilities:

* Send WAHA/GOWA owner alert.
...
* [ ] User can open `/` and see KMB / Kursus Mobil Bantul brand.
* [ ] User can understand the business is a local driving course in Bantul/Yogyakarta.
* [ ] User can navigate to pricing, schedule, about, FAQ, and contact sections.
* [ ] User can see active package cards loaded from Supabase.
* [ ] User can distinguish `mobil_kursus` and `mobil_sendiri` package types.
* [ ] User can start booking from package CTA.
* [ ] Package CTA redirects to `/booking?packageId={id}`.
* [ ] Floating WhatsApp button is visible and uses the correct number.
* [ ] Landing page is responsive on mobile, tablet, and desktop.
* [ ] SEO title and description are present.
* [ ] LocalBusiness JSON-LD is present.
...
* [ ] Valid `packageId` query param preselects package.
* [ ] Invalid `packageId` does not break booking flow.
* [ ] User can select date starting from tomorrow.
* [ ] User cannot select today or past dates.
* [ ] User can select one of four slots: `08:00`, `10:00`, `13:00`, `15:00`.
* [ ] Slot with existing `tentative` or `scheduled` session is disabled.
* [ ] Slot with `canceled` session is not blocked.
* [ ] User can fill name, WhatsApp number, pickup point, and notes.
* [ ] User cannot proceed if required fields are invalid.
* [ ] User must upload payment proof before submit.
* [ ] Invalid file type is rejected.
* [ ] File larger than 5 MB is rejected.
* [ ] Honeypot submission is rejected.
* [ ] Rate limit blocks excessive submission.
* [ ] Successful submission creates/updates student.
* [ ] Successful submission creates booking with status `pending_verification`.
* [ ] Successful submission uploads payment proof to private storage.
* [ ] Successful submission creates payment with status `pending_verification`.
* [ ] Successful submission creates one session with status `tentative`.
* [ ] Success state shows booking code.
* [ ] Success state explains that admin will verify payment and confirm schedule via WhatsApp.
* [ ] Success state shows WhatsApp CTA with prefilled booking code.

---

## 19.3 WhatsApp Integration

* [ ] Manual WhatsApp CTA works even when WAHA/GOWA is disabled.
* [ ] If WA automation is enabled, owner receives booking alert.
* [ ] If WA automation is enabled, customer receives auto-reply.
* [ ] If WA notification fails, booking remains successful.
* [ ] WA failure is logged for debugging.

---

## 19.4 Admin Hub Readiness

* [ ] Public app writes are compatible with documented tables.
* [ ] Status semantics are documented and consistent.
* [ ] Package records can be managed externally.
* [ ] Payment proof is stored as private `proof_path`.
* [ ] Admin Hub can later verify payment, confirm booking, create sessions, and manage packages.
...
* Setup private payment proof storage.
* Setup package fetching.
* Setup booking submission route.
* Setup server-side validation.
* Setup rate limiting.
* Generate human-readable booking code.
* Create one tentative first session.
* Add WhatsApp CTA.
